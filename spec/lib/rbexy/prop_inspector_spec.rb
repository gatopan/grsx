...
RSpec.describe Rbexy::PropInspector do
  ...
  describe ".scan_tree" do
    def parse(source)
      template = Rbexy::Template.new(source, "test.rbx")
      tokens = Rbexy::Lexer.new(template, Rbexy.configuration.element_resolver).tokenize
      Rbexy::Parser.new(tokens).parse
    end
