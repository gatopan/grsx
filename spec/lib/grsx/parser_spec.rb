require "timeout"
RSpec.describe Grsx::Parser do
  it "handles :TEXT" do
    subject = Grsx::Parser.new([[:TEXT, "Hello world"]])
    result = subject.parse.children
    expect(result.first).to be_a Grsx::Nodes::Text
    expect(result.first.content).to eq "Hello world"
  end

  it "parses declarations" do
    subject = Grsx::Parser.new([
      [:DECLARATION, "<!DOCTYPE html>"]
    ])
    result = subject.parse.children
    expect(result.first).to be_a Grsx::Nodes::Declaration
  ...
  it "parses expressions" do
    ...
    result = subject.parse.children
    expect(result.first).to be_a Grsx::Nodes::ExpressionGroup
    group = result.first
    expect(group.members.first).to be_a Grsx::Nodes::Expression
  ...
  it "parses tags within expressions" do
    ...
    expect(group.members[0]).to be_a Grsx::Nodes::Expression
    expect(group.members[0].content).to eq "true && "

    expect(group.members[1]).to be_a Grsx::Nodes::HTMLElement
    expect(group.members[1].name).to eq "h1"
    expect(group.members[1].children[0]).to be_a Grsx::Nodes::Text
    expect(group.members[1].children[0].content).to eq "Is "
    expect(group.members[1].children[1]).to be_a Grsx::Nodes::ExpressionGroup
    expect(group.members[1].children[1].members[0].content).to eq "'hello'.upcase"

    expect(group.members[2]).to be_a Grsx::Nodes::Expression
  ...
  it "parses named tags" do
    ...
    result = subject.parse.children
    expect(result.first).to be_a Grsx::Nodes::HTMLElement
  ...
  it "raises if tag is missing a name" do
    ...
    expect { subject.parse }.to raise_error Grsx::Parser::ParseError
  end
  ...
  it "parses tag attributes" do
    ...
    expect(div).to be_a Grsx::Nodes::HTMLElement
    expect(div.members.length).to eq 3

    attrFoo = div.members[0]
    expect(attrFoo).to be_a Grsx::Nodes::HTMLAttr
    expect(attrFoo.name).to eq "foo"
    expect(attrFoo.value).to be_a Grsx::Nodes::Text
    expect(attrFoo.value.content).to eq ""

    attrBar = div.members[1]
    expect(attrBar).to be_a Grsx::Nodes::HTMLAttr
    expect(attrBar.name).to eq "bar"
    expect(attrBar.value).to be_a Grsx::Nodes::Text
    expect(attrBar.value.content).to eq "baz"

    attrThing = div.members[2]
    expect(attrThing).to be_a Grsx::Nodes::HTMLAttr
    expect(attrThing.name).to eq "thing"
    expect(attrThing.value).to be_a Grsx::Nodes::ExpressionGroup
  ...
  it "parses splat attributes" do
    ...
    attrFoo = div.members[0]
    expect(attrFoo).to be_a Grsx::Nodes::ExpressionGroup
  ...
  it "finds no children for self-closing tags" do
    ...
    expect(input).to be_a Grsx::Nodes::HTMLElement
  ...
  it "parses text within a tag into Grsx::Nodes::Text" do
    subject = Grsx::Parser.new([
      [:OPEN_TAG_DEF],
      [:TAG_DETAILS, { name: "div", type: :html }],
      [:CLOSE_TAG_DEF],
    ...
    expect(text).to be_a Grsx::Nodes::Text
    expect(text.content).to eq "Hello world"
  end

  it "parses expression within a tag into Grsx::Nodes::Expression" do
    subject = Grsx::Parser.new([
      [:OPEN_TAG_DEF],
      [:TAG_DETAILS, { name: "div", type: :html }],
      [:CLOSE_TAG_DEF],
    ...
    expect(expr).to be_a Grsx::Nodes::ExpressionGroup
  ...
  it "raises an error when encountering tag that opens but never closes" do
    ...
    expect { Timeout::timeout(1) { subject.parse } }
      .to raise_error(Grsx::Parser::ParseError)
  end
